...
class MiniPlugin {
        ...
        fileList.forEach(file => {
            try {
                ...
                if (isQuickApp) {
                    ...
                    const importCustomComponents = helper_1.getImportCustomComponents(this.outputDir, depComponents);
                    const usingComponents = configObj.usingComponents;
                ...
                if (isNative) {
                    ...
                    if (fs.existsSync(stylePath))
                        taroFileTypeMap[file.path]['style'] = fs.readFileSync(stylePath).toString();
                }
    ...
    generateMiniFiles(compilation) {
        ...
        Object.keys(taroFileTypeMap).forEach(item => {
            ...
            const templatePath = relativePath.replace(extname, constants_1.MINI_APP_FILES[buildAdapter].TEMPL).replace(/\\/g, '/');
            const jsonPath = relativePath.replace(extname, constants_1.MINI_APP_FILES[buildAdapter].CONFIG).replace(/\\/g, '/');
            const scriptPath = relativePath.replace(extname, constants_1.MINI_APP_FILES[buildAdapter].SCRIPT).replace(/\\/g, '/');
            const stylePath = relativePath.replace(extname, constants_1.MINI_APP_FILES[buildAdapter].STYLE).replace(/\\/g, '/');
            ...
            if (template && (!this.changedFile
                || this.changedFile === item
                || this.changedFileType === constants_1.PARSE_AST_TYPE.ENTRY
    ...
    watchRun(compiler, changedFiles) {
        ...
        if (constants_1.REG_SCRIPTS.test(changedFile)) {
            ...
            if (this.changedFileType === constants_1.PARSE_AST_TYPE.ENTRY
                ...
                if (obj && type === constants_1.PARSE_AST_TYPE.COMPONENT && !this.components.has(obj)) {
